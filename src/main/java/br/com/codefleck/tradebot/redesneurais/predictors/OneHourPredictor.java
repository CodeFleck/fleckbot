...
public class OneHourPredictor {
    ...
    public List<String> predictOneHour(String simbolo, String period, int batchSize, double splitRatio, PriceCategory category, int epochs, double learningRate) throws IOException {

        List<String> dataPointsList;

        StopWatch watch = new StopWatch();
        OneHourStockDataSetIterator oneHourIterator = predictionService.getOneHourStockDataSetIterator(simbolo, predictionService.getFileForTrainingNeuralNets(period), batchSize, splitRatio, category);
        ...
        File locationToSave = new File("/Users/dfleck/projects/tcc/fleckbot-11-09-2017/fleckbot/src/main/resources/onehour/StockPriceLSTM_".concat(period).concat(String.valueOf(category)).concat(".zip"));
        ...
        if (category.equals(PriceCategory.ALL)) {
            ...
            predictionService.predictAllCategories(oneHourNet, test, max, min);
            ...
            dataPointsList = predictionService.predictPriceOneAhead(oneHourNet, test, max, min);
            log.info(period + " done testing...");
            System.out.println("Time Elapsed: " + watch.getTime());
        }
